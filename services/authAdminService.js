...
import { UserStaffModel } from '../models/index.js';
...
const createNewGoogleUser = async ({ id, email, firstName, lastName, profilePhoto }) => {
    const oldUser = await UserStaffModel.findOne({ email: email.toLowerCase() });
    if (oldUser)
        throw new APIError(httpStatus.BAD_REQUEST, "Email already exists.")
    const newUser = await UserStaffModel.create({ email, source: "google" });
    if (!newUser)
        throw new APIError(httpStatus.BAD_REQUEST, "Oops...seems our server needed a break!")
    return newUser;
}
...
const fetchUserFromEmailAndPassword = async ({ email, password }) => {
    const user = await UserStaffModel.findOne({
...
const verifyUserFromRefreshTokenPayload = async ({ userId }) => {
    const userExists = await UserStaffModel.exists({
        _id: userId,
    });
...
const updatePassword = async (userId, newPassword) => {
    ...
    let user = await UserStaffModel.findOneAndUpdate(
        {
            _id: userId,
        },
        {
